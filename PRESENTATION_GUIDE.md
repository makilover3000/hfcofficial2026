...
> Yes — the construction site locations are from HDB's official open dataset, and the weather/air quality readings are fetched live from Singapore government APIs (data.gov.sg). The sensor readings at each site are simulated for this prototype since we don't have physical IoT sensors deployed, but the AI logic that processes them is fully functional.

**"How would this work in production?"**
> Each construction site would have IoT noise and dust sensors streaming data to our backend. The AI agents would process these readings in real-time — the same code that runs in our demo, just with real sensor inputs instead of simulated ones. Town councils would get push notifications for critical alerts and could approve or override from their phones.

**"Why are some numbers static?"**
> For this showcase, we focused engineering effort on the core AI coordination loop — the part that detects cumulative impacts and coordinates across sites. Analytics dashboard numbers like "1,247 total decisions" represent what the system would show after running in production for several days. We prioritized making the core agent pipeline fully functional over populating every dashboard metric.

**"What APIs would you need?"**
> NEA real-time air quality and noise monitoring APIs, data.gov.sg weather APIs (already integrated), BCA construction project data for activity types and schedules, HDB population data for accurate resident impact estimates, and an IoT sensor platform (e.g. MQTT broker) for streaming site-level readings.

**"What makes this different from just monitoring?"**
> Existing systems monitor individual sites in isolation. UrbanPulse detects cumulative impacts — when Site A at 78dB and Site B at 85dB are both near the same HDB blocks, the combined impact on residents is far worse than either alone. Our AI coordinates across sites: staggering schedules, redistributing work intensity, and activating suppression systems. No human supervisor has visibility across all sites simultaneously.
